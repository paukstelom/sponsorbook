...
@router.get("/", response_description="Get sub_organization", response_model=List[SubOrganization])
async def get_sub_organizations_endpoint():
    return [item async for item in get_sub_organizations(database=db)]


@router.get(
    "/{sub_organization_id}", response_description="Get one sub_organization", response_model=SubOrganization
)
async def get_one_sub_organization_endpoint(sub_organization_id: str):
    return await get_sub_organization(sub_organization_id=sub_organization_id, database=db)
